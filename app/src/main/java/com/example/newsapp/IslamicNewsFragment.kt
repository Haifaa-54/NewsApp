...
class IslamicNewsFragment : Fragment() {
...
var newsArray=ArrayList<NewsItem>()
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_islamic_news, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        GlobalScope.launch(Dispatchers.IO) {var data=
            NewsDatabase.getDatabase(view.context)?.getNewsDao()?.getNewsItems(3,20)
...
//
//        })
        var rv=view.findViewById<RecyclerView>(R.id.islamicNewsRV)
        adapter= newsAdapter(newsArray,object:newsAdapter.INewsListener{
            override fun itemViewListener(position: Int) {
                ...
                findNavController().navigate(IslamicNewsFragmentDirections.actionIslamicNewsFragmentToNewsItemListFragment(articalTitle,imageUrl,articalUrl))
            }
